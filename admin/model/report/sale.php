...
class ModelReportSale extends Model {
    ...
    public function getReportOrders($data = array()) {
        $sql = "SELECT op.*, o.order_status_id, o.firstname, o.lastname, o.date_added
        ...
        if (!empty($data['filter_order_product_id'])) {
            $sql .= " AND op.product_id = '" . (int)$data['filter_order_product_id'] . "'";
        }else {
            $sql .= " AND op.product_id > '0'";
        }

        if (!empty($data['filter_id_start'])) {
            $sql .= " AND o.order_id >= '" . (int)$data['filter_id_start'] . "'";
        }

        if (!empty($data['filter_id_end'])) {
            $sql .= " AND o.order_id <= '" . (int)$data['filter_id_end'] . "'";
        }
    ...
    public function getTotalReportOrders($data = array()) {
        ...
        if (!empty($data['filter_date_start'])) {
            ...
            $sql .= " AND DATE(o.date_added) >= CURDATE() - INTERVAL 1 DAY";
        }

        if (!empty($data['filter_date_end'])) {
            $sql .= " AND DATE(o.date_added) <= '" . $this->db->escape($data['filter_date_end']) . "'";
        }else{
            $sql .= " AND DATE(o.date_added) <= CURDATE()";
        }
