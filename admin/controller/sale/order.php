...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['send_email'] = $this->url->link('sale/order/sendEmail', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function sendEmail(){
        ...
        if ($this->config->get('config_alert_mail')) {
            ...
            $this->load->model('sale/order');
            $order_info = $this->model_sale_order->getOrder($order_id);
            $subject = sprintf("%s - Đơn hàng mới %s", html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_id);
            $text  = $order_info['payment_firstname'] . " " . $order_info['payment_lastname']. " " . $order_info['payment_address_1'] . " " . $order_info['payment_city'] . " " . $order_info['payment_zone_code'] . " " . $order_info['payment_postcode'] . "\n\n";

            $order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");

            foreach ($order_product_query->rows as $product) {
                $text .= "Mua " . $product['quantity'] . ' thùng '. $product['model'] . "\n";
